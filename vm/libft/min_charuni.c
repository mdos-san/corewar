...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/04/05 11:47:23 by jcao              #+#    #+#             */
/*   Updated: 2016/04/05 11:53:28 by jcao             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	str_left(t_pri var, char *convc)
{
	ft_putstr(convc);
...
static int	str_right(t_pri var, char *convc)
{
	var.tmp2 = ft_strnew(var.len - 1);
	var.tmp2 = space_count((var.len - 1), var.tmp2);
	ft_putstr(var.tmp2);
...
'7' || sub[i] == '8' || sub[i] == '9' || sub[i] == '0')
	...
	var.ret = var.ret + ft_strlen(convc) - 1;
	return (var.ret);
}
